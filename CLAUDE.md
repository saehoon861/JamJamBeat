...
│   │   ├── mlp_baseline/      ← 각 파이프라인: model.py + dataset.py
│   │   ├── mlp_baseline_full/
│   │   ├── mlp_baseline_seq8/
│   │   ├── mlp_sequence_joint/
│   │   ├── mlp_sequence_delta/
...
run_pipeline.py  ──imports──▶  _shared.py
        │                          (JOINT_COLS, BONE_COLS, SplitData,
        │                           FrameDataset, SequenceDataset, etc.)
        │
        ├── load_preprocessed_data()  ← data_fusion/*_output.csv 읽기
        ├── split_by_group()          ← source 파일 단위로 train/val/test 분리
        │
...
```
run_all.py
  └── subprocess.run([run_pipeline.py, --model-id, {model_id}, ...])  × 13회 순차
        ↓ 각 실행 완료 후
...
모든 파이프라인이 공통으로 임포트하는 파일. **절대 독립적으로 수정하면 안 됨** — 변경 시 전체 13개 파이프라인에 영향.
...
- 주요 하이퍼파라미터 기본값: epochs=20, batch=128, lr=1e-3, patience=6, seq_len=8

### `model/model_pipelines/run_all.py`
전체 모델 순차 실행기.

- `ALL_MODELS` 리스트: 실행 순서 (새 파이프라인 추가 시 여기에도 등록 필수)
- 각 모델을 subprocess로 격리 실행 → GPU 메모리 완전 해제
- 결과: `model/model_evaluation/pipelines/comparison_results.csv`
...
4. `dataset.py` — 반드시 아래 시그니처의 `build()` 함수 포함:
   ...
   ```
   `mode` 값: `"frame"` / `"two_stream"` / `"sequence"` / `"image"` 중 하나
5. `run_pipeline.py`의 `MODEL_CHOICES`에 추가
6. `run_all.py`의 `ALL_MODELS`에 추가

---

## 입력 데이터 규격

**전처리 완료 CSV 경로 (DEFAULT_INPUTS):**
```
...
**전처리 파이프라인:**
```
원시 CSV (x0,y0,z0,...x20,y20,z20)
    ↓
model/data_fusion/hand_preprocess.py
    ↓
*_output.csv (nx*, ny*, nz*, bx*, by*, bz*, bl*, flex_*, abd_*)
```

**피처 차원:**
```
JOINT_COLS:  63d  (21 joints × nx/ny/nz)
...
|----------|------|----------|------|
| `mlp_baseline` | frame | 63d | 기준선 |
| `mlp_baseline_full` | frame | 156d | 동일 아키텍처, bone/angle 추가 효과 검증 |
| `mlp_baseline_seq8` | sequence | 8×63→504 | 동일 아키텍처, T=8 temporal context 효과 검증 |
| `mlp_sequence_joint` | sequence | 16×63 | MLP + flatten (큰 hidden dim) |
| `mlp_sequence_delta` | sequence | 16×126 | joint + 1차 차분 |
| `mlp_temporal_pooling` | sequence | T×63 | mean/max/std pooling |
| `mlp_embedding` | frame | 156d | LayerNorm+GELU embedding head |
| `two_stream_mlp` | two_stream | 63d + 93d | 두 스트림 분리 입력 |
...
# 일부 모델만
python model/model_pipelines/run_all.py --models mlp_baseline mlp_baseline_full mlp_baseline_seq8
...
- **`_shared.py` 수정 시**: 모든 13개 파이프라인에 영향 → backward compatibility 반드시 확인
- **새 파이프라인 추가 시**: `run_pipeline.py:MODEL_CHOICES` + `run_all.py:ALL_MODELS` 양쪽 등록 필수
- **`dataset.py`의 `build()` 반환 타입**: `(nn.Module, str, Dataset, Dataset, Dataset)` 순서 고정
- **mode 문자열**: `"frame"` / `"two_stream"` / `"sequence"` / `"image"` 외 값 사용 불가 (`forward_batch`에서 ValueError)
