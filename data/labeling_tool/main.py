"""main.py - [Controller] 라벨링 툴 진입점 및 이벤트 루프 오케스트레이션

실행 방법:
    JamJamBeat/ 디렉토리에서:
    $ cd data/labeling_tool && uv run python main.py
"""
...
def get_project_paths() -> tuple[str, str]:
    """프로젝트 경로를 계산하여 (raw_data_dir, labeled_data_dir)를 반환합니다."""
    tool_dir = Path(__file__).resolve().parent       # data/labeling_tool/
    data_dir = tool_dir.parent                       # data/
    raw_data_dir = str(data_dir / "raw_data")
    labeled_data_dir = str(data_dir / "labeled_data")
...
def main():
    ...
    print("=" * 60)
    print("  🎮 Hand Gesture Frame Labeling Tool")
    print("=" * 60)
