...
* **수정 내용**:
  ...
  * **키보드 입력 방어 로직 재설계 (우선순위 부여)**:
    ...
    2. **대기 모드 방어**: 현재 `is_waiting_for_class()` 상태라면 오직 `0~6` 숫자키, `S`(취소), `Esc` 세 가지만 허용. 이외의 입력(방향키, E, Q, 스페이스바, Enter 등)은 모두 무시(`continue`).
...
- [ ] **Data Exporter 행(Row) 길이 불일치 방어**: `landmark_df`의 행 개수와 `labels`의 개수가 코덱 문제로 1프레임 정도 다를 수 있음. `landmark_df.iloc[:limit_frames].copy()` 처럼 길이를 잘라낸 뒤 컬럼을 삽입해야 `ValueError: Length of values does not match length of index`를 막을 수 있음.
- [ ] **`fps` 파라미터 전달 확인**: `labeled_data`를 백지에서 생성할 때, OpenCV에서 읽어온 실제 동영상의 프레임 레이트(`video.fps`)가 `data_exporter`의 `format_timestamp()`까지 물 흐르듯 전달되어야 함. 하드코딩된 `30`을 쓰지 않도록 주의.
- [ ] **Data Loader 안전망 유지**: 반환 객체 `SessionData` 타입 정의(14줄 부근) 멤버 중 `labeled_df`의 타입 힌트를 `pd.DataFrame | None` 타입으로 변경하여 IDE 경고 방지 및 설계 정합성 유지. 
- [ ] **동시성 문제와 Side Effect 차단**: `total_check_tool`은 기존 `landmark_extractor`나 원본 파일을 절대 망치면 안 됨. `SessionData`에서 꺼내온 `landmark_df`를 `total_data`용으로 조작하기 전 무조건 `.copy()` 호출 (in-memory 오염 방지).