...
const DEFAULT_GESTURE_VOTE_N = 5;
const DEFAULT_GESTURE_DEBOUNCE_K = 3;
const DEFAULT_GESTURE_CLEAR_FRAMES = 2;
const DEFAULT_GRAB_MOTION_THRESHOLD = 0.028;
...
function normalizeModelLabel(rawLabel) {
  ...
  if (label === "grab" || label === "class7" || label === "class 7" || label === "7") return "Grab"; // grab 클래스도 브라우저 표시용 이름으로 맞춥니다.
  return "None"; // 어디에도 해당하지 않으면 인식 실패로 처리합니다.
}

function getGestureVoteN() {
  return Math.round(parseNumberParam("gestureVoteN", DEFAULT_GESTURE_VOTE_N, 1, 9));
}

function getGestureDebounceK() {
  return Math.round(parseNumberParam("gestureDebounceK", DEFAULT_GESTURE_DEBOUNCE_K, 1, 9));
}

function getGestureClearFrames() {
  return Math.round(parseNumberParam("gestureClearFrames", DEFAULT_GESTURE_CLEAR_FRAMES, 1, 6));
}

function getGrabMotionThreshold() {
  return parseNumberParam("grabMotionThreshold", DEFAULT_GRAB_MOTION_THRESHOLD, 0.001, 0.3);
}

function getPredictionMotionScore(modelPrediction) {
  return Number.isFinite(modelPrediction?.recent_motion_score)
    ? modelPrediction.recent_motion_score
    : 0;
}
...
function applyGrabMotionGate(rawResult, modelPrediction) {
  const motionScore = getPredictionMotionScore(modelPrediction);

  if (rawResult.label !== "Grab") {
    return {
      ...rawResult,
      motionScore,
      motionGateBlocked: false
  ...
  if (motionScore < getGrabMotionThreshold()) {
    return {
      label: "None",
      confidence: 0,
      source: "motion-gate",
      motionScore,
      motionGateBlocked: true
    };
  }
...
function pushVoteSample(stableState, sample) {
  stableState.voteHistory.push(sample);
  const voteLimit = getGestureVoteN();
  if (stableState.voteHistory.length > voteLimit) {
    stableState.voteHistory.splice(0, stableState.voteHistory.length - voteLimit);
  }
}

function resolveVotedResult(stableState) {
  const history = stableState.voteHistory.slice(-getGestureVoteN());
  if (!history.length) {
    return {
      label: "None",
      confidence: 0,
  ...
  const winner = [...history].reverse().find((entry) => counts.get(entry.label) === maxCount);
  if (!winner) {
    return {
      label: "None",
      confidence: 0,
...
function stabilize(rawResult, stableState, modelPrediction) {
  const gatedResult = applyGrabMotionGate(rawResult, modelPrediction);
  pushVoteSample(stableState, gatedResult);
  const votedResult = resolveVotedResult(stableState);

  stableState.motionGateBlocked = gatedResult.motionGateBlocked;
  stableState.motionScore = gatedResult.motionScore;
  stableState.votedLabel = votedResult.label;

  if (votedResult.label === "None") {
    stableState.noneFrames += 1;
    stableState.debounceLabel = "None";
    stableState.debounceFrames = 0;

    if (stableState.noneFrames >= getGestureClearFrames()) {
      stableState.stableLabel = "None";
      stableState.confidence = 0;
      stableState.source = votedResult.source;
    }
  } else {
    stableState.noneFrames = 0;

    if (stableState.stableLabel === votedResult.label) {
      stableState.confidence = votedResult.confidence;
      stableState.source = votedResult.source;
      stableState.debounceLabel = votedResult.label;
      stableState.debounceFrames = 0;
    } else {
      if (stableState.debounceLabel === votedResult.label) {
        stableState.debounceFrames += 1;
      } else {
        stableState.debounceLabel = votedResult.label;
        stableState.debounceFrames = 1;
      }

      if (stableState.debounceFrames >= getGestureDebounceK()) {
        stableState.stableLabel = votedResult.label;
        stableState.confidence = votedResult.confidence;
        stableState.source = votedResult.source;
      }
    }
  ...
  return {
    ...
    motionScore: stableState.motionScore,
    motionGateBlocked: stableState.motionGateBlocked,
    votedLabel: stableState.votedLabel,
    debouncedLabel: stableState.stableLabel
  };
}
...
export function resolveGesture(landmarks, now, sessionStarted, handKey = "default") {
  ...
  return stabilize(rawResult, stableState, modelPrediction); // 마지막으로 흔들리는 결과를 안정화해서 돌려줍니다.
}
