...
import { DEFAULT_SOUND_MAPPING, loadCustomSounds, loadSoundMapping, getSoundProfileForInstrument, loadGestureMapping } from "./sound_mapping.js";
...
import { createInstrumentAnimationManager } from "./instrument_animations.js";
...
const instrumentElements = { // 각 악기 식물들의 HTML 요소를 하나로 묶어둡니다.
  drum: document.getElementById("instrumentA"), // 드럼은 a 오브젝트 DOM을 같이 사용합니다.
  ...
  a: document.getElementById("instrumentA") // a 오브젝트 DOM입니다.
};
const instrumentAImage = instrumentElements.a?.querySelector(".instrument-art") || null;
let gestureVariantFlip = false;
let gestureObjectActive = false;
let lastGestureVariantSwapAt = 0;
const GESTURE_VARIANT_SWAP_MS = 260;
...
const feverController = createFeverController({
  ...
  onFeverEnd: () => {
    animationManager.setFeverMode(false);
  }
});
...
function setGestureObjectVariant(isGestureActive) {
  if (!instrumentAImage) return;
  let nextSrc = instrumentAImage.dataset.variantBase;
  const now = performance.now();

  if (isGestureActive) {
    if (!gestureObjectActive) {
      nextSrc = gestureVariantFlip
        ? instrumentAImage.dataset.variantGestureA
        : instrumentAImage.dataset.variantGestureB;
      gestureVariantFlip = !gestureVariantFlip;
      lastGestureVariantSwapAt = now;
    } else {
      const shouldSwap = now - lastGestureVariantSwapAt >= GESTURE_VARIANT_SWAP_MS;
      if (shouldSwap) {
        nextSrc = gestureVariantFlip
          ? instrumentAImage.dataset.variantGestureA
          : instrumentAImage.dataset.variantGestureB;
        ...
        nextSrc = instrumentAImage.getAttribute("src") || instrumentAImage.dataset.variantBase;
      }
    }
  }

  gestureObjectActive = Boolean(isGestureActive);
  if (!gestureObjectActive) {
    lastGestureVariantSwapAt = 0;
  }
  instrumentAImage.classList.toggle("is-gesture-variant", gestureObjectActive);
  if (!nextSrc || instrumentAImage.getAttribute("src") === nextSrc) return;
  instrumentAImage.setAttribute("src", nextSrc);
}
...
async function initMediaPipe() {
  ...
  try {
    handLandmarker = await HandLandmarker.createFromOptions(vision, {
      ...
      numHands: NUM_HANDS // 기본 1개 손 추적, 필요 시 URL numHands=2로 확장합니다.
    });
  } catch (delegateError) {
    ...
    handLandmarker = await HandLandmarker.createFromOptions(vision, {
      ...
      numHands: NUM_HANDS // 손 개수도 동일합니다.
    });
  }
}
...
function bindUIEvents() {
  ...
  const measureAudioUnlock = async (reason) => {
    ...
    let unlocked = false;
    try {
      unlocked = await Audio.unlockAudioContext();
    } catch (error) {
  ...
  landingStartButton.onclick = async () => {
    ...
    try {
      ...
      activateStart(); // 오디오 오류가 있어도 화면 전환은 막지 않습니다.
    }
  };
...
const trackingRuntime = createHandTrackingRuntime({
  ...
  onBeforeFrame: (now, started) => {
    ...
    if (feverController.isFever() && started) {
      if (Math.random() < 0.025) { // 피버 버블 생성 확률을 낮춰 렌더 부하를 줄입니다.
