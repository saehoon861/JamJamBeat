...
const REQUEST_TIMEOUT_MS = 500; // 배포환경: 모델 응답 여유 확보
const DEFAULT_REQUEST_INTERVAL_MS = 150; // 배포환경: 반응성 향상 (초당 ~6.7회)
const FAIL_OPEN_AFTER = 5;
const DISABLE_FOR_MS = 800; // 배포환경: 실패 후 복구 대기 단축
const PERF_ENABLED = (() => {
...
function flushPerf(now = performance.now()) {
  ...
  console.info("[Perf][ModelInference]", {
    ...
    timeouts: perfWindow.timeoutCount,
  ...
  fetch(endpoint, {
    ...
    .catch((error) => {
      ...
      if (PERF_ENABLED) {
        ...
        perfWindow.totalMs += elapsedMs;
        perfWindow.maxMs = Math.max(perfWindow.maxMs, elapsedMs);
        if (error?.name === "AbortError") perfWindow.timeoutCount += 1;
      }
