...
const DEFAULT_GRAB_MOTION_WINDOW = 4;
...
function createDisabledPrediction() {
  return {
    ...
    disabled: true,
    status: "disabled",
    recent_motion_score: 0,
    motion_window: getGrabMotionWindow(),
    raw_pred_index: 0,
    raw_pred_label: "None"
  };
}
...
function getGrabMotionWindow() {
  const raw = Number(new URLSearchParams(window.location.search).get("grabMotionWindow"));
  if (!Number.isFinite(raw)) return DEFAULT_GRAB_MOTION_WINDOW;
  return Math.max(1, Math.min(SEQUENCE_LENGTH - 1, Math.round(raw)));
}
...
function computeRecentMotionScore(buffer, motionWindow = getGrabMotionWindow()) {
  if (!Array.isArray(buffer) || buffer.length < 2) {
    return 0;
  }

  const stepCount = Math.min(Math.max(1, motionWindow), buffer.length - 1);
  const startIndex = buffer.length - stepCount;
  let scoreSum = 0;
  ...
  for (let index = startIndex; index < buffer.length; index += 1) {
    ...
    scoreSum += Math.sqrt(meanSquare / 63);
  }

  return scoreSum / stepCount;
}
...
function normalizePrediction(
  ...
  tau = DEFAULT_TAU,
  recentMotionScore = 0,
  motionWindow = getGrabMotionWindow()
) {
  ...
  return {
    ...
    recent_motion_score: recentMotionScore,
...
async function scheduleModelRequest(landmarks, now, handKey = "default") {
  ...
  try {
    ...
    const motionWindow = getGrabMotionWindow();
    const recentMotionScore = computeRecentMotionScore(handState.jointBuffer, motionWindow);
